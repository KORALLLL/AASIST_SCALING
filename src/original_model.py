...
class SSLModel(nn.Module):
    def __init__(self):
        ...
        # Загрузка предварительно обученной модели с HuggingFace Hub
        self.model = Wav2Vec2Model.from_pretrained("facebook/wav2vec2-xls-r-300m")

        # Размерность выходного вектора модели (для xls-r-300m это 1024)
        self.out_dim = self.model.config.hidden_size
        return

    def extract_feat(self, input_data):

        # Перемещаем модель на GPU, если она еще не там
        if next(self.model.parameters()).device != input_data.device \
                or next(self.model.parameters()).dtype != input_data.dtype:
            self.model.to(input_data.device, dtype=input_data.dtype)

        # Сохраняем логику режима обучения, как в оригинальном коде
        self.model.train()

        # Входные данные должны иметь форму (batch, length)
        if input_data.ndim == 3:
            # Убираем последнюю размерность, если она есть (например, (batch, length, 1))
            input_tmp = input_data.squeeze(-1)
        else:
            input_tmp = input_data

        # Получаем эмбеддинги из модели HuggingFace
        # [batch, length, dim]
